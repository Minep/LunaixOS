...
static bool _RD_keyb_BatRES = false;
static bool _RD_keyb_DiagRES = false;
static bool _RD_keyb_ResendRES = false;
static bool _RD_keyb_DISABLE = false;
static int _RD_keyb_ScanC_STD[] = {
	//! key			scancode
	KEY_UNKNOWN,	//0
	KEY_ESCAPE,		//1
	KEY_1,			//2
...
uint8_t P_RD_CRStatus();
void P_RD_CSCmd(uint8_t);
uint8_t P_RD_ENCRBuf();
void P_RD_ENCSCmd(uint8_t);
void _cdecl RAINBOW_keyB_IRQ();

uint8_t P_RD_CRStatus()
{
	return LUNA_PortIN(KYBRD_CTRL_STATS_REG);
}

void P_RD_CSCmd(uint8_t cmd)
{
	while (1)
	{
		if ((P_RD_CRStatus()&KYBRD_CTRL_STATS_MASK_IN_BUF) == 0)
		{
			break;
		}
	}
	LUNA_PortOUT(KYBRD_CTRL_CMD_REG, cmd);
}

uint8_t P_RD_ENCRBuf()
{
	return LUNA_PortIN(KYBRD_ENC_INPUT_BUF);
}

void P_RD_ENCSCmd(uint8_t cmd)
{
	while (1)
	{
		if ((P_RD_CRStatus()&KYBRD_CTRL_STATS_MASK_IN_BUF) == 0)
	...
	LUNA_PortOUT(KYBRD_ENC_CMD_REG, cmd);
}
...
void _cdecl RAINBOW_keyB_IRQ()
{
	...
	if (P_RD_CRStatus() & KYBRD_CTRL_STATS_MASK_OUT_BUF) {
		...
		if (code == 0xE0 || code == 0xE1)
			...
			if (code & 0x80) {	//test bit 7
				...
				//! grab the key
				int key = _RD_keyb_ScanC_STD[code];
				...
				case KEY_CAPSLOCK:
					...
					RD_keyb_setLED(_numlock, _capslock, _scrolllock);
					break;
		...
		case KYBRD_ERR_BAT_FAILED:
			_RD_keyb_BatRES = false;
			break;

		case KYBRD_ERR_DIAG_FAILED:
			_RD_keyb_DiagRES = false;
			break;

		case KYBRD_ERR_RESEND_CMD:
			_RD_keyb_ResendRES = true;
			break;
		}
	}
...
bool RD_keyb_getScrollLock()
{
	return _scrolllock;
}

bool RD_keyb_getNumLock()
{
	return _numlock;
}

bool RD_keyb_getCapsLock()
{
	return _capslock;
}
...
bool RD_keyb_getCTRL()
{

	return _ctrl;
}

bool RD_keyb_getSHIFT()
{
	return _shift;
}

void RD_keyb_IGN_RESEND()
{
	_RD_keyb_ResendRES = false;
}

bool RD_keyb_CHK_RESEND()
{
	return _RD_keyb_ResendRES;
}

bool RD_keyb_getDiagnosticRES()
{
	return _RD_keyb_DiagRES;
}

bool RD_keyb_getBatRES()
{
	return _RD_keyb_BatRES;
}

uint8_t RD_keyb_getLastScanCode()
{
	return _scancode;
}

void RD_keyb_setLED(bool num, bool cap, bool scr)
{
	uint8_t dat = 0;
	dat = (scr) ? (dat | 1) : (dat & 1);
	dat = (num) ? (dat | 2) : (dat & 2);
	...
	P_RD_ENCSCmd(KYBRD_ENC_CMD_SET_LED);
	P_RD_ENCSCmd(dat);
}

RAINBOWDASH_KEYCODE RD_keyb_getLastKey()
{
	return (_scancode != INVALID_SCANCODE) ? ((RAINBOWDASH_KEYCODE)_RD_keyb_ScanC_STD[_scancode]) : (KEY_UNKNOWN);
}

void RD_keyb_DiscardLastKey()
{
	_scancode = INVALID_SCANCODE;
}

char RD_keyb_ToASCII(RAINBOWDASH_KEYCODE rdc)
{
	uint8_t key = rdc;
...
void RD_keyb_DISABLE()
{
	P_RD_CSCmd(KYBRD_CTRL_CMD_DISABLE);
	_RD_keyb_DISABLE = true;
}

void RD_keyb_ENABLE()
{
	P_RD_CSCmd(KYBRD_CTRL_CMD_ENABLE);
	_RD_keyb_DISABLE = false;
}

bool RD_keyb_isDisabled()
{
	return _RD_keyb_DISABLE;
}

void RD_keyb_RESSYS()
{
	P_RD_CSCmd(KYBRD_CTRL_CMD_WRITE_OUT_PORT);
	P_RD_ENCSCmd(0xfe);
}

bool RD_keyb_SelfEval()
{
	P_RD_CSCmd(KYBRD_CTRL_CMD_SELF_TEST);
	while (1)
	{
		if ((P_RD_CRStatus()&KYBRD_CTRL_STATS_MASK_OUT_BUF) == 0)
		{
			break;
		}
	}
	return (P_RD_ENCRBuf() == 0x55) ? true : false;
}

void RD_keyb_Initialize(int irq)
{
	LUNA_set_INTvect(irq, RAINBOW_keyB_IRQ);
	_RD_keyb_BatRES = true;
	_scancode = 0;
	_numlock = _scrolllock = _capslock = false;
	RD_keyb_setLED(false, false, false);
